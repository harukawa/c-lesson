...
int to_mnemonic_symbol(char *str, int len) {
	...
	char *mnemonic;
	mnemonic = malloc(len);
	memcpy(mnemonic, str, len);
	value = search_name_tree(mnemonic, &mnemonic_root);
	if(value != -1) {
		return value;
	} else {
		mnemonic_id++;
		value = add_name_tree(mnemonic, &mnemonic_root, mnemonic_id);
		return value;
	}
}
...
void setup_mnemonic() {
	init();
	raw = to_mnemonic_symbol(".raw", 4);
	mov = to_mnemonic_symbol("mov", 3);
	MOV = to_mnemonic_symbol("MOV", 3);
	ldr = to_mnemonic_symbol("ldr", 3);
	LDR = to_mnemonic_symbol("LDR", 3);
	str = to_mnemonic_symbol("str", 3);
	STR = to_mnemonic_symbol("STR", 3);
